...
def make_vector(val, dim=4):
    # Create a vector with val on first element, zeros elsewhere, shape [1, dim]
    v = torch.zeros(dim)
    v[0] = val
    return v.unsqueeze(0)

def make_orthogonal_vector(idx, dim=4):
...
def test_add_duplicate_skip():
    ...
    cache.add(v1 * 1.01, 2)  # Slightly different vector, should be skipped as duplicate
...
def test_is_near():
    ...
    v1 = make_vector(1)
    cache.add(v1, 1)
    assert cache.is_near(v1).item()
    v_far = torch.tensor([[0., 1., 0., 0.]])
    assert not cache.is_near(v_far).item()
...
def test_top_k_updates_timestamp():
    ...
    counter_before = cache._counter
    dist, idx = cache.top_k(v1)
    assert dist.shape == (1, 1)
    assert idx.shape == (1, 1)
    assert cache._counter == counter_before + 1
    updated_idx = idx.item()
    assert cache._last_used[updated_idx] == cache._counter

# Test that fit properly resets the cache database, labels, timestamps, and counter
def test_fit_resets_cache():
    cache = LRUCache(capacity=10)
    vectors = torch.cat([make_vector(i) for i in range(3)], dim=0)  # Shape (3, dim)
    labels = [10, 20, 30]
    cache.fit(vectors, labels)
    assert len(cache) == 3
    assert cache.labels == labels
    assert torch.equal(cache._last_used, torch.arange(3))
    assert cache._counter == 3

# Test that fit raises a ValueError if vectors and labels have mismatched lengths
def test_fit_raises_on_mismatch():
    cache = LRUCache()
    vectors = torch.cat([make_vector(i) for i in range(2)], dim=0)
    labels = [1]
    with pytest.raises(ValueError):
        cache.fit(vectors, labels)
